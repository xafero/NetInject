...
namespace NetInject
{
    class Island
    {
        ...
        internal static int Replace(IsleOptions opts)
        {
            ...
            {
                var tuples = FindInstructions(file, rparam, term).ToArray();
                if (tuples.Length < 1)
                    continue;
                ...
                foreach (var tuple in byMethod)
                {
                    ...
                    foreach (var instr in instrs)
                    {
                        ...
                        var patch = CreateOperation(proc, opts.Patch);
        ...
        static Instruction CreateOperation(ILProcessor proc, string patch)
        {
